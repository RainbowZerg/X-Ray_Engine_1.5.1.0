...
//		float			fBias				= (SE_SUN_NEAR==sub_phase)?(-ps_r2_sun_depth_near_bias):ps_r2_sun_depth_far_bias;
		...
		if ((SE_SUN_NEAR == sub_phase || SE_SUN_MIDDLE == sub_phase))
 		...
 		else if (!ps_r2_ls_flags.is(R2FLAG_SUN_ZCULLING))
			HW.pDevice->SetRenderState(D3DRS_ZFUNC, D3DCMP_ALWAYS);
		else
...
{
	// ZergO: it's already checked before function call
	//if (!(RImplementation.o.advancedpp && ps_r_sun_shafts)) return;
	if (g_pGamePersistent->Environment().CurrentEnv->m_fSunShaftsIntensity < 0.0001) return;

	//	Test. draw only for near part
//	if (sub_phase!=SE_SUN_NEAR) return;
