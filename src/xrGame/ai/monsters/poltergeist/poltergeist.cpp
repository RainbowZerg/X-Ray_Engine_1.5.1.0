...
CPoltergeist::CPoltergeist()
{
	...
	m_ability_impulse			= 0;
	m_ability_impulse_radius	= 0;
	m_ability_trace_attempts	= 0;
	m_ability_trace_distance	= 0;
}
...
void CPoltergeist::Load(LPCSTR section)
{
	...
	m_ability_impulse			= READ_IF_EXISTS(pSettings, r_float, section, "Ability_Impulse", IMPULSE);
	m_ability_impulse_radius	= READ_IF_EXISTS(pSettings, r_float, section, "Ability_Impulse_Radius", IMPULSE_RADIUS);
	m_ability_trace_attempts	= READ_IF_EXISTS(pSettings, r_u32, section, "Ability_Trace_Attempts", TRACE_ATTEMPT_COUNT);
	m_ability_trace_distance	= READ_IF_EXISTS(pSettings, r_float, section, "Ability_Trace_Distance", TRACE_DISTANCE);
}
