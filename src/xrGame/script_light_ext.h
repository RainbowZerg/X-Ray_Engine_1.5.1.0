...
// Flare
void CLASS_IMPL::SetFlare(bool b, int target)
{
	switch (target)
	{
	case 0: target_0->set_flare(b);
	case 1: if (target_1) target_1->set_flare(b);
	}
}

// Volumetric
void CLASS_IMPL::SetVolumetric(bool b, int target)
{
	switch (target)
	{
	case 0: target_0->set_volumetric(b);
	case 1: if (target_1) target_1->set_volumetric(b);
	}
}
void CLASS_IMPL::SetVolumetricIntensity(float f, int target)
{
	switch (target)
	{
	case 0: target_0->set_volumetric_intensity(f);
	case 1: if (target_1) target_1->set_volumetric_intensity(f);
	}
}
void CLASS_IMPL::SetVolumetricQuality(float f, int target)
{
	switch (target)
	{
	case 0: target_0->set_volumetric_quality(f);
	case 1: if (target_1) target_1->set_volumetric_quality(f);
	}
}
void CLASS_IMPL::SetVolumetricDistance(float f, int target)
{
	switch (target)
	{
	case 0: target_0->set_volumetric_distance(f);
	case 1: if (target_1) target_1->set_volumetric_distance(f);
	}
}
