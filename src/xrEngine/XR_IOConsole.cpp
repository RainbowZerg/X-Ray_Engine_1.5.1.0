...
//const char *				ch_cursor_old		= ch_cursor;
...
/*
void CConsole::Change_cursor() // ZergO - меняем курсор по кнопке Insert
{
	if (!stricmp(ch_cursor, ch_cursor_old))
		ch_cursor = "|";
	else
		ch_cursor = "_";
}
...
bool CConsole::is_mark(Console_mark type)
{
	...
	case mark8:  case mark9:  case mark10: case mark11:	case mark12: return true; break;
	}
	return false;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CConsole::OnFrame()
{
	...
	if (Device.dwFrame % 10 == 0) // ZergO - CoP
		update_tips(); 
}
...
void CConsole::OnRender()
{
	...
	if ((g_pGameLevel && g_pGameLevel->bReady) || (g_pGamePersistent && g_pGamePersistent->m_pMainMenu && g_pGamePersistent->m_pMainMenu->IsActive())) bGame = true;
#endif
...
//	float dwMaxX			= float(Device.dwWidth/2);
	...
	float d1				= pFont->SizeOf_(ch_cursor);
...
void CConsole::DrawBackgrounds(bool bGame)
{
	...
	if (m_select_tip < (int)m_tips.size())
	{
		...
		for (u32 i = 0; itb != ite; ++itb, ++i ) // tips
		{
			...
			if ((ts.HL_start < 0) || (ts.HL_finish < 0) || (ts.HL_start > ts.HL_finish)) continue;

			int str_size		= (int)ts.text.size();
			if ((ts.HL_start >= str_size) || (ts.HL_finish > str_size)) continue;
...
void CConsole::ExecuteCommand(LPCSTR cmd_str, bool record_cmd)
{
	...
	if (it != Commands.end())
	{
		...
		if (cc && cc->bEnabled)
		{
			if (cc->bLowerCaseArgs) strlwr(last);
	...
	{
		first[CONSOLE_BUF_SIZE-21] = 0; // ZergO - "frame stack corruption" bug ?
...
void CConsole::Show()
{
	...
	scroll_delta				= 0;
	reset_cmd_history_idx		();
	reset_selected_tip			(); // ZergO - CoP
	update_tips					(); // ZergO - CoP
...
{
	if (!bVisible)										return;
	//if (g_pGamePersistent && g_dedicated_server)		return; // ZergO - watch next line
#ifdef DEDICATED_SERVER
	if (g_pGamePersistent)								return;
#endif
//	if (!g_pGamePersistent->m_pMainMenu->IsActive() && !g_pGameLevel) return;
...
bool CConsole::add_next_cmds(LPCSTR in_str, vecTipsEx& out_v)
{
	...
	if (cur_count >= MAX_TIPS_COUNT) return false;
	...
	if (!cc || temp.size() == 0) return false;
	...
	for (u32 i = cur_count; i < MAX_TIPS_COUNT*2; ++i) //fake=protect
	{
		...
		if (out_v.size() >= MAX_TIPS_COUNT) break;
		LPSTR t3;
...
bool CConsole::add_internal_cmds(LPCSTR in_str, vecTipsEx& out_v)
{
	...
	for (; itb != ite; ++itb)
	{
		...
		if (out_v.size() >= MAX_TIPS_COUNT) return res;
	}
...
void CConsole::select_for_filter(LPCSTR filter_str, vecTips& in_v, vecTipsEx& out_v)
{
	...
	if (in_count == 0 || !filter_str) return;
